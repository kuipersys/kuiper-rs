...
use crate::model::admission_policy::{AdmissionOperation, FailurePolicy};
use anyhow::Context;
...
#[async_trait]
impl ValidationCommand for AdmissionWebhookCommand {
    async fn validate(&self, ctx: &CommandContext) -> CommandResult {
        ...
        let value: Value = match ctx.parameters.get("value") {
            Some(v) => v.clone(),
            None => return Ok(None),
        };

        let api_version = match value.get("apiVersion").and_then(|v| v.as_str()) {
            Some(v) => v.to_string(),
            None => return Ok(None),
        };
...
fn build_auth_headers(
    auth: &crate::model::service_endpoint::ServiceAuth,
) -> anyhow::Result<HeaderMap> {
    use crate::model::service_endpoint::ServiceAuth;
