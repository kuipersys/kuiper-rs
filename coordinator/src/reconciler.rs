...
impl ReconcilerService {
            ...
            tokio::select! {
                ...
                _ = debounce_timer.tick() => {
                    if pending_reconcile {
                        ...
                        tracing::info!("⚡ Debounced reconcile trigger — executing locally.");
                ...
                _ = backup_timer.tick() => {
                    tracing::debug!("⏰ Backup 30s reconcile — executing locally.");
                    if let Err(e) = self.reconcile_once().await {
                        tracing::error!("Reconciliation failed: {:#}", e);
                    }
                }
    ...
    fn handle_message(&self, msg: ServerMessage) -> bool {
        match msg {
            ...
            ServerMessage::Event { resource, namespace, action, object } => {
